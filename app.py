...
# Configure logging - setting level to DEBUG for more detailed logs
...
@app.route('/')
def home():
    app.logger.debug("Entering home route") # Log entry to home route
    return render_template('index.html')

@app.route('/training')
def training():
    app.logger.debug("Entering /training route") # Log entry to training route
    ...
    if 'game_state' not in session:
        ...
        session['game_state'] = {
            ...
            'board': {
                ...
                'middle': [None] * 5, # 5 слота для средней линии
    ...
    return render_template('training.html', game_state=session['game_state']) # Corrected line: render training.html

@app.route('/update_state', methods=['POST'])
def update_state():
    app.logger.debug("Entering /update_state route") # Log entry to update_state route
    ...
    try:
        ...
        app.logger.debug(f"Received game state update: {game_state}") # Log received game state
        ...
        app.logger.debug(f"Session AFTER update: {session['game_state']}") # Log session state after update
...
def ai_move():
    ...
    app.logger.debug("Entering ai_move route") # Log entry to ai_move route
    game_state_data = request.get_json()
    app.logger.debug(f"Received game state data for AI move: {game_state_data}") # Log received game state data
    ...
    try:
        ...
        app.logger.debug(f"Processed selected_cards: {selected_cards}") # Log processed selected cards
        ...
        app.logger.debug(f"Processed discarded_cards: {discarded_cards}") # Log processed discarded cards
        ...
        app.logger.debug(f"Created game state: {game_state}") # Log created game state
        ...
        app.logger.debug(f"Next available slots BEFORE AI call: {next_available_slots}") # Log next available slots
    ...
    try:
        ...
        if move is None or 'error' in move:
            app.logger.error(f"AI move error: {move.get('error', move.get('error', 'Unknown error'))}") # Log AI move error
            return jsonify({'error': move.get('error', 'Unknown error')}), 500
        app.logger.debug(f"AI move received: {move}") # Log AI move data
    except Exception as e:
    ...
    try:
        ...
        app.logger.debug(f"Serialized move: {serialized_move}") # Log serialized move
        ...
        if move:
            ...
            app.logger.debug(f"Updated session with new board: {session['game_state']['board']}") # Log updated session board

        app.logger.debug(f"Returning AI move: {serialized_move}, Royalties: {royalties}, Total Royalty: {total_royalty}") # Log returned AI move data
        return jsonify({
